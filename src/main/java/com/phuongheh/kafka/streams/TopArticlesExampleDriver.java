...
public class TopArticlesExampleDriver {
    ...
    private static void consumeOutput(String bootstrapServers, String schemaRegistryUrl) {
        ...
        final KafkaConsumer<Windowed<String>, String> consumer = new KafkaConsumer<Windowed<String>, String>(consumerProperties,
                windowedDeserializer, Serdes.String().deserializer());

        consumer.subscribe(Collections.singleton(TopArticlesLambdaExample.TOP_NEW_PER_INDUSTRY_TOPIC));
        while (true) {
    ...
    private static Schema loadSchema(String name) throws IOException {
        try (final InputStream input = TopArticlesLambdaExample
                .class
                .getClassLoader()
